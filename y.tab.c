...
/*
   var <variable_name> “:” <range_from> “..” <range_to> “;” -> definition
   <expression> (is | is not) <number> “;” -> constraint
*/
...
enum yysymbol_kind_t
{
  ...
  YYSYMBOL_definition = 23                 /* definition  */
};
...
{
  "\"end of file\"", "error", "\"invalid token\"", "NUMBER", "VAR", "V",
  "POINT", "IS", "IS_NOT", "'='", "'+'", "'-'", "'*'", "'/'", "';'", "'('",
  "')'", "':'", "$accept", "program", "statement", "constraint",
  "expression", "definition", YY_NULLPTR
};
...
{
       0,    -8,    -8,    -3,     3,     2,     0,    -6,    16,    -5,
      -7,     9,    -8,    -8,    -8,     8,     3,     3,     3,    -8,
      14,    -8,    -8,    19,    12,    12,    -8,    23,    -8,    27,
      -8
};
...
{
      11,    10,    12,     1,     2,     3,     1,     2,    14,    19,
      20,    22,    24,    25,    26,     4,    23,    27,     4,    16,
      17,    18,    28,    15,    18,    21,    16,    17,    18,    29,
      30,    13
};

static const yytype_int8 yycheck[] =
{
       4,     4,     0,     3,     4,     5,     3,     4,    14,    14,
      17,     3,    16,    17,    18,    15,     8,     3,    15,    10,
      11,    12,     3,     7,    12,    16,    10,    11,    12,     6,
       3,     6
};
...
{
       0,     3,     4,     5,    15,    19,    20,    21,    22,    23,
       4,    22,     0,    19,    14,     7,    10,    11,    12,    14,
      17,    16,     3,     8,    22,    22,    22,     3,     3,     6,
       3
};
...
yyreduce:
  ...
  switch (yyn)
    {
  case 4: /* statement: definition ';'  */
#line 58 "solver.yacc"
                                                          { }
#line 1181 "y.tab.c"
    break;

  case 5: /* statement: constraint ';'  */
#line 59 "solver.yacc"
                                                                      { }
#line 1187 "y.tab.c"
    break;

  case 6: /* constraint: expression IS NUMBER  */
#line 62 "solver.yacc"
                                              {
                                                array_trees[count_trees]= (yyvsp[-2].node);
                                                constraint_v[count_trees]=(yyvsp[0].num);
                                                count_trees++;
                                              }
#line 1197 "y.tab.c"
    break;

  case 7: /* constraint: expression IS IS_NOT NUMBER  */
#line 67 "solver.yacc"
                                                 {
                                                not[count_trees]=1;
                                                array_trees[count_trees]= (yyvsp[-3].node);
                                                constraint_v[count_trees]=(yyvsp[0].num);
                                                count_trees++;
                                              }
#line 1208 "y.tab.c"
...
#line 1220 "y.tab.c"
  ...
  case 9: /* expression: VAR  */
#line 83 "solver.yacc"
                               {
                                  if(is_used[(yyvsp[0].id)]==0){
                                     printf ("variable %c not defined",'a'+(yyvsp[0].id));
                                     exit(-1);
                                   }
                                  Node *n1 = malloc(sizeof(Node));
                                  n1->op=N;
                                  n1->number=(yyvsp[0].id);
                                  n1->old_var=-1;
                                  (yyval.node) = n1;
                                }
#line 1236 "y.tab.c"
...
#line 1275 "y.tab.c"
  ...
  case 14: /* definition: V VAR ':' NUMBER POINT NUMBER  */
#line 121 "solver.yacc"
                                             {
                                  assert((yyvsp[-4].id)>=0 && (yyvsp[-4].id)<NUM_VARS);
        												  if(is_used[(yyvsp[-4].id)]==1){
        												  	printf ("variable is already declared\n");
                                    exit(-1);
        												  }
        												  else{
                                    count_vars++;
        												  	int size= (yyvsp[0].num) - (yyvsp[-2].num) + 1;
                                    definition_size[(yyvsp[-4].id)]=size;
        												  	vars_values[(yyvsp[-4].id)]= (int*)malloc((size)*sizeof(int));
        												  	is_used[(yyvsp[-4].id)]=1;
        												  	int i,num= (yyvsp[-2].num);
        												  	for(i=0;i<size;i++){
        												  		vars_values[(yyvsp[-4].id)][i]=num++;
        												 	 }
                                   int n= (yyvsp[0].num),count=0;
                                   while (n != 0) {
                                      n /= 10;     // n = n/10
                                      ++count;
                                  }
...
void put_digits(int *digit_array,int number){
  int temp=NUM_VARS;
  int n = number;
  while(temp-- != 0){
    if(is_used[temp] != 0){
      digit_array[temp] = n % 10;
      n /= 10;
    }
  }
}
...
void put12(int *digit_array,int n){
  int i;
  int count_digits=1;
  int number ;
  for(i=0;i<NUM_VARS;i++){
  ...
  while(temp-- != 0){
    if(is_used[temp] != 0){
      ...
      digit_array[temp] = m;
      /* printf("%c =%d ",'a'+temp,digit_array[temp]); */
      x=0;
      m=0;
    }
  }
...
int main (void) {
  if(yyparse()!=-1){
    /* int digit_array[NUM_VARS]={0};
    put12(digit_array,1123);
    find_max_digit(); */
